	...
	if (holds_alternative<Literal>(_e))
	{
		...
		if (
			Expression const* repr =
				RepresentationFinder(m_dialect, m_meter, locationOf(_e)->irLocation, m_cache)
...
{
	Representation repr;
	repr.expression = make_unique<Expression>(Literal{make_shared<DebugData>(m_location), LiteralKind::Number, YulString{formatNumber(_value)}, {}});
...
) const
{
	...
	repr.expression = make_unique<Expression>(FunctionCall{
		make_shared<DebugData>(m_location),
		Identifier{make_shared<DebugData>(m_location), _instruction},
